...
export function buildReportHtml(report, user) {
  ...
  const body = `
    <div class="page">
      ...
      <div class="footer">Generated by FSR Demo • ${esc(generatedAt)} • ${esc(buildLabel)}</div>
    </div>
  `;
...
export function exportFieldPictures(report) {
  ...
  const footer = `<p style="margin-top:16px;color:#6b7280;font-size:12px;">Generated by FSR Demo • ${esc(
    generatedAt,
  )} • ${esc(buildLabel)}</p>`;
  const body = `<h1>${esc(title)}</h1>${grid}${footer}`;
